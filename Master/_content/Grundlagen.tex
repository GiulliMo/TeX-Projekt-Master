...
%			node [] {};
	...
	\label{sec: Bestimmung der Positionskoordinaten}
		...
		Abbildung \ref{fig: positionsbestimmung} zeigt ein Fallbeispiel für die Positionserkennung. Die $z$-Koordinate ist als Höhenkoordinate zu interpretieren. Der Bildsensor der Kamera befindet sich im Ursprung des gezeigten Koordinatensystems. Zwischen der Objektivebene und dem Bildsensor besteht eine absolute Distanz $x_f$. Diese wird auch als Brennweite bezeichnet. Der Sichtwinkel $\gamma$ der Kamera spannt sich zwischen den Begrenzungslinien $f_l$ und $f_r$ auf. Das Objekt, dessen longitudinalen $x_loc$ und lateralen $y_loc$ Positionskoordinaten im Bezug zum Bildsensor bestimmt werden sollen, ist in der Abbildung mit einem Punkt gekennzeichnet und entsprechend beschrieben. Durch die Variable $d$ wird die absolute Distanz des Bildsensors zum Objekt beschrieben.\\
			...
			\caption{Funktionsprinzip der Positionsbestimmung durch ein zweidimensionales Bild und Tiefeninformationen von oben betrachtet. Der Bildsensor der Kamera befindet sich im Ursprung des Koordinatensystems. Somit ist die Brennweite der Abstand zwischen der als gepunktete Linie dargestellte Objektivebene und des Bildsensors. Diese ist hier mit $x_f$ gekennzeichnet. Der aufgespannte Bereich zwischen den Geraden $f_l$ und $f_r$ stellt den Sichtbereich der Kamera dar. Durch eine gestrichelte Linie ist hier die absolute Distanz der Kamera zum Objekt gezeigt.}
			\label{fig: positionsbestimmung}
		\end{figure}
		
		Zur Berechnung der longitudinalen Positionskoordinate $x_loc$ wird das mathematische Verhältnis aus Gleichung \ref{eq: verhaeltnis} definiert. Der Teil der Distanz $d$, der durch den Bildsensor und der Objektivebene begrenzt wird, wird mit der Variable $d'$ besetzt. Dieser wird aus Darstellungsgründen nicht in der vorangegangenen Abbildung berücksichtigt. \\
		...
		Die Brennweite kann bei vielen Kamera´modellen häufig aus dem Datenblatt entnommen werden. Jedoch lässt sich ebenfalls durch ihre Lage zu der Objektivebene beschreiben. Hierfür wird der Sichtwinkel $\gamma$ und die Breite des Bildes $b$ verwendet. Gleichung \ref{eq: focal} zeigt das bereits nach der Brennweite umgestellte Verhältnis. 
		
		\begin{equation}
			x_f=\frac{b}{2\cdot\tan\left(\frac{\gamma}{2}\right)}
			\label{eq: focal}
		\end{equation}\\
		
		Gleichung \ref{eq: dstrich} zeigt die Berechnung von $d'$. Die Kenntnis über die Position des Objekts als Pixelkoordintane des Bildes ist dafür notwendig. Somit ergibt sich für die Positionskoordinaten.
