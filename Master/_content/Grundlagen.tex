...
\label{ch: Grundlagen}
	...
	Anders als bei regelbasierten Systemen verhalten sich KNNs grundlegend verschieden \cite{proba}. Sie lernen Verhaltensmuster basierend auf den entsprechenden Trainingsdatensatz \cite{proba}. Folglich bildet ein Datensatz die reale Anwendungen ab \cite{training}.\\
	
	Häufig kann der reale Einsatz jedoch nicht vollständig abgebildet werden\cite{dataset}. Hierbei genügen kleine Abweichungen hinsichtlich der Trainingsaten im Vergleich zu den Eingangsdaten, die während des Einsatzes im Feld aufgenommen werden.\\
	
	Somit kann nur bedingt eine Aussage darüber getroffen werden, welches Verhalten durch das KNN erzeugt wird, wenn das KNN im realen Umfeld eingesetzt wird. Unter diesen Umständen kann es zu unvorhersehbaren Ergebnissen kommen. Am vorliegenden ALF werden zur Personenerkennung derartige neuronale Netze untersucht.
